...
public final class MetaHolder {
    ...
    public <T> void set(MetadataDef.@NotNull Entry<T> entry, T value) {
        ...
        switch (entry) {
            ...
            case MetadataDef.Entry.BitMask mask -> this.entries.compute(id, (integer, currentEntry) -> {
            ...
            case MetadataDef.Entry.ByteMask mask -> this.entries.compute(id, (integer, currentEntry) -> {
                byte maskValue = currentEntry != null ? (byte) currentEntry.value() : 0;
                maskValue = setMaskBit(maskValue, (byte) mask.byteMask(), (Boolean) value);
                return Metadata.Byte(maskValue);
            });
        }
    ...
    public <T> T get(MetadataDef.@NotNull Entry<T> entry) {
        ...
        return switch (entry) {
            ...
            case MetadataDef.Entry.ByteMask mask -> {
                ...
                yield (T) ((Boolean) getMaskBit(maskValue, (byte) mask.byteMask()));
            }
