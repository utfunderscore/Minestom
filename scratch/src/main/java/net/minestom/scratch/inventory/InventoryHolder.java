...
public final class InventoryHolder {
    ...
    public WindowItemsPacket itemsPacket() {
        return ScratchInventoryUtils.makePlayerPacket(inventory, cursor);
    }
        ...
        for (ClientClickWindowPacket.ChangedSlot changedSlot : changedSlots) {
            ...
            if (openContainer != null) {
                ...
                if (protocolSlot < containerSize) {
                    ...
                    final int internalSlot = PlayerInventoryUtils.convertWindowSlotToMinestomSlot(protocolSlot, containerSize);
                    this.inventory[internalSlot] = changedSlot.item();
                }
            } else {
                // No container open
                final int internalSlot = PlayerInventoryUtils.convertMinestomSlotToWindowSlot(protocolSlot);
                this.inventory[internalSlot] = changedSlot.item();
    ...
    public void consume(ClientCreativeInventoryActionPacket packet) {
        final int internalSlot = PlayerInventoryUtils.convertPlayerInventorySlotToMinestomSlot(packet.slot());
    ...
    private int equipmentSlot(EquipmentSlot equipmentSlot) {
        return switch (equipmentSlot) {
            ...
            default -> equipmentSlot.armorSlot();
        };
    }
