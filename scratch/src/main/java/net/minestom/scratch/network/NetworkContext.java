...
public sealed interface NetworkContext {
    ...
    default void write(SendablePacket packet) {
    ...
    sealed interface Packet {
        record PacketIdPair(SendablePacket packet) implements Packet {
        }
    ...
    static void write(NetworkBuffer buffer, PacketParser<ServerPacket> parser, ConnectionState state,
        ...
        switch (packet) {
            case NetworkContext.Packet.PacketIdPair packetPair -> {
                final ServerPacket packetLoop = SendablePacket.extractServerPacket(state, packetPair.packet);
