package net.minestom.scratch.template;
...
public final class LimboTemplate {
    ...
    static final class Connection {
        ...
        void handlePacket(ClientPacket packet) {
            ...
            switch (packet) {
                ...
                case ClientLoginStartPacket startPacket -> {
                    ...
                    this.networkContext.write(new LoginSuccessPacket(new GameProfile(uuid, username, List.of())));
                }
                case ClientLoginAcknowledgedPacket ignored -> {
                    for (SendablePacket registryPacket : ScratchRegistryTools.instance.getRegistryPackets()) {
        ...
        private final ImmutableChunkRepeatWorld world = new ImmutableChunkRepeatWorld(ScratchRegistryTools.instance.dimensionType().get(DimensionType.OVERWORLD), ScratchRegistryTools.instance.biome(), unit -> unit.modifier().fillHeight(0, 48, Block.STONE));
        ...
        final ScratchFeature.Messaging messaging = new ScratchFeature.Messaging(new ScratchFeature.Messaging.Mapping() {
            ...
            public Component formatMessage(String message) {
                return Component.text(username).color(TextColor.color(0x00FF00)).append(Component.text(" > ")).append(Component.text(message));
            }
        ...
        void init() {
            ...
            final DynamicRegistry.Key<DimensionType> dimensionKey = ScratchRegistryTools.instance.dimensionType().getKey(dimension);
            final int dimensionId = ScratchRegistryTools.instance.dimensionType().getId(dimensionKey);

            this.networkContext.write(
                    new JoinGamePacket(id, false, List.of(), 0, VIEW_DISTANCE,
                            VIEW_DISTANCE, false, true, false,
                            dimensionId, dimensionKey.name(), 0, GameMode.CREATIVE, null,
                            false, true, new WorldPos(dimensionKey.name(), Vec.ZERO),
                            0, 100, false));


            this.networkContext.write(new SpawnPositionPacket(position, 0));
            this.networkContext.write(new PlayerPositionAndLookPacket(0, position, Pos.ZERO, 0, 0, RelativeFlags.DELTA_COORD));
            this.networkContext.write(new PlayerInfoUpdatePacket(
                    EnumSet.of(PlayerInfoUpdatePacket.Action.ADD_PLAYER, PlayerInfoUpdatePacket.Action.UPDATE_LISTED),
                    List.of(new PlayerInfoUpdatePacket.Entry(uuid, username, List.of(), true, 1,
            ...
            ChunkRange.chunksInRange(position.chunkX(), position.chunkZ(), VIEW_DISTANCE, (x, z) -> networkContext.write(world.chunkPacket(x, z)));
        ...
        void processPacket(ClientPacket packet) {
            ...
            this.chunkLoading.accept(packet);
            {
                ...
                final PlayerListHeaderAndFooterPacket listHeaderAndFooterPacket = new PlayerListHeaderAndFooterPacket(Component.text("Welcome to Minestom Limbo!"), Component.text("Heap: " + heapUsage / 1024 / 1024 + "MB"));
